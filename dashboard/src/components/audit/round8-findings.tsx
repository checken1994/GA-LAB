...
export function Round8Findings() {
  return (
    <section
      id="round8-audit"
      className="scroll-mt-20 border-b border-border/40 bg-gradient-to-b from-amber-500/[0.03] to-transparent py-14"
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        <div className="mb-8">
          ...
          <h2 className="mt-1 flex items-center gap-2 text-2xl font-bold tracking-tight sm:text-3xl">
            ...
            Round 8 Findings · {R8_STATS.total} NEW root-cause bugs
          </h2>
          <p className="mt-2 max-w-3xl text-sm text-muted-foreground">
            DNA #22 recursive: &ldquo;R7 found 14 bugs&rdquo; doesn&apos;t mean 14 were the
            ONLY bugs. R8 ran the strengthened scanners + world&apos;s-best-tool patterns
            (ruff / bandit / semgrep / vulture / pyright / CodeQL signatures) and found{" "}
            <strong className="text-amber-600 dark:text-amber-400">
              {R8_STATS.total} NEW bugs R7/R7-Full MISSED
            </strong>{" "}
            — across {R8_STATS.bugClasses.length} bug classes. ALL patched in real Python.{" "}
            <span className="text-emerald-600 dark:text-emerald-400">
              {R8_STATS.astParseOk}/{R8_STATS.total} ast.parse OK.
            </span>
          </p>
        </div>
          ...
          <Card className="border-fuchsia-500/30 bg-fuchsia-500/5 p-4">
            ...
            <p className="mt-2 text-sm font-bold tabular-nums">
              <span className="text-amber-600 dark:text-amber-400">
                {R8_STATS.bySeverity.high}
              </span>
              {" / "}
              <span className="text-yellow-600 dark:text-yellow-400">
                {R8_STATS.bySeverity.medium}
              </span>
              {" / "}
              <span className="text-sky-600 dark:text-sky-400">
                {R8_STATS.bySeverity.low}
              </span>
            </p>
          </Card>
        </div>
        ...
        <Card className="mb-10 border-border/60 bg-background/40 p-5 sm:p-6">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground">
              How R8 found them — world&apos;s-best-tool patterns via grep + ast
            </h3>
          </div>
          <ol className="space-y-3">
            {R8_METHODOLOGY.map((m, i) => {
                ...
                <div className="flex flex-col gap-4 p-5 sm:p-6">
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="space-y-3">
                      ...
                      <div>
                        <p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                          Why R7 / R7-Full missed it
                        </p>
                        <p className="mt-0.5 text-xs text-muted-foreground">{f.whyR7Missed}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="space-y-1.5">
                      ...
                      <pre className="max-h-56 overflow-auto rounded-md border border-rose-500/20 bg-rose-500/[0.04] p-3 text-[11px] leading-relaxed text-muted-foreground scroll-thin">
                    ...
                    <div className="space-y-1.5">
                      <p className="flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wider text-emerald-600 dark:text-emerald-400">
                        <CheckCircle2 className="h-3 w-3" />
                        After (R8 patch — real .py)
                      </p>
                      <pre className="max-h-56 overflow-auto rounded-md border border-emerald-500/20 bg-emerald-500/[0.04] p-3 text-[11px] leading-relaxed text-foreground scroll-thin">
        ...
        {/* Closing note */}
        <div className="mt-10 rounded-lg border border-amber-500/30 bg-amber-500/[0.04] p-5">
          <p className="text-xs leading-relaxed text-muted-foreground">
            <strong className="text-amber-600 dark:text-amber-400">
              DNA #22 applied recursively to the bug-finder:
            </strong>{" "}
            R7 found 14 bugs and R7-Full patched them. R8 did NOT trust that 14 was
            exhaustive — it re-ran the scanners + world-tool patterns and found{" "}
            {R8_STATS.total} more. Every one is patched in real Python with ast.parse
            verification. A Round 9 bug-finder would find bugs R8 missed — that is
            the feature, not the failure. (DNA #23 + #26.)
          </p>
        </div>
      </div>
    </section>
