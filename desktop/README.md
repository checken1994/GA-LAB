...
API kiểm tra trạng thái gồm `GET http://127.0.0.1:8002/v3/pc/status` và `GET http://127.0.0.1:8002/v3/web/status`. Web Navigator có thể đọc nguồn công khai qua `POST /v3/web/browse`; AI Orchestrator dùng `POST /v3/ai/ask` với `approved=true` khi cần điều khiển phiên browser đã đăng nhập. Cross-verification dùng `POST /v3/ai/cross-verify` và không xem sự đồng thuận của nhiều AI là bằng chứng cuối cùng.
