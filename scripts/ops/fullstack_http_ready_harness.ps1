...
$override = @{
  ...
  "SCP_BASE_URL" = "http://127.0.0.1:8001"
...
try {
  ...
  Wait-Http "backend" "http://127.0.0.1:8001/health" 180 | Out-Null
  $results.Add((Probe "backend_health" "GET" "http://127.0.0.1:8001/health"))
  $results.Add((Probe "backend_detailed" "GET" "http://127.0.0.1:8001/health/detailed"))
  $results.Add((Probe "backend_auth_negative" "POST" "http://127.0.0.1:8001/v105/autofix/run-audit"))
  ...
  $assert = [ordered]@{
    ...
    scheduler_backend_8001 = $schedulerText.Contains("scp=http://127.0.0.1:8001")
  ...
  foreach ($port in @(3000,3031,8001,11435)) { $report.teardown_ports[$port.ToString()] = $false }
  ...
  foreach ($port in @(3000,3031,8001,11435)) { Get-NetTCPConnection -LocalPort $port -State Listen -ErrorAction SilentlyContinue | Select-Object -ExpandProperty OwningProcess -Unique | ForEach-Object { Stop-Process -Id $_ -Force -ErrorAction SilentlyContinue } }
  Start-Sleep -Seconds 2
  foreach ($port in @(3000,3031,8001,11435)) { $free = -not [bool](Get-NetTCPConnection -LocalPort $port -State Listen -ErrorAction SilentlyContinue); Write-Output ("PORT_" + $port + "_FREE=" + $free) }
}
