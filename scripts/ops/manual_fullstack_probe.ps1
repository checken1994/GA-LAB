...
$replace=@{SCP_ENV_FILE=$EnvFile;SCP_SIDECAR_ENV_FILE=$EnvFile;SCP_EGRESS_MODE='deny';SCP_SKIP_STARTUP_GATE='1';SCP_BASE_URL='http://127.0.0.1:8001';LLM_BRIDGE_URL='http://127.0.0.1:11435'}
...
try{
  ...
  Start-Child backend $py @('-m','scp') $Root|Out-Null;Wait-OK 'http://127.0.0.1:8001/health';$results+=Probe backend_health GET 'http://127.0.0.1:8001/health';$results+=Probe backend_detailed GET 'http://127.0.0.1:8001/health/detailed';$results+=Probe backend_auth_negative POST 'http://127.0.0.1:8001/v105/autofix/run-audit' @{'Authorization'='Bearer invalid-test-token'}
  ...
  $summary=[ordered]@{evidence=$Evidence;results=$results;bridge_env=($logs.Contains("env source=$EnvFile"));scheduler_env=($logs.Contains("env source=$EnvFile"));scheduler_8001=$logs.Contains('scp=http://127.0.0.1:8001');no_lan_url=(-not $logs.Contains('192.168.'));egress_mode='deny';error=$null}
