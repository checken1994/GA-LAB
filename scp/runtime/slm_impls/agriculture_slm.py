"""
[OPT-20] AgricultureSLM — DomainExpert for agriculture (uses existing AgricultureDataSource).
...
class AgricultureSLM(BaseSLM):
    ...
    def __init__(self, config: Optional[dict] = None):
        super().__init__(name="AgricultureSLM", domain="agriculture", config=config)
        self._ds = None
        try:
            from scp.data_sources.agriculture import AgricultureDataSource
            self._ds = AgricultureDataSource()
        except Exception as e:
            logger.debug(f"AgricultureSLM AgricultureDataSource init: {e}")
    ...
    def predict(self, question: str) -> SLMResponse:
        ...
        if not answer and self._ds:
            try:
                ...
                logger.debug(f"AgricultureSLM DataSource query: {e}")
...
AgricultureExpert = AgricultureSLM
