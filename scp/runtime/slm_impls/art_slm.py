"""
[OPT-21] ArtSLM — DomainExpert for art (uses existing ArtsDataSource).
...
NOTE: Distinct from existing ArtsSLM (_DomainSLM) — this version has explicit
...
class ArtSLM(BaseSLM):
    ...
    def __init__(self, config: Optional[dict] = None):
        super().__init__(name="ArtSLM", domain="arts", config=config)
        self._ds = None
        try:
            from scp.data_sources.arts import ArtsDataSource
            self._ds = ArtsDataSource()
        except Exception as e:
            logger.debug(f"ArtSLM ArtsDataSource init: {e}")
    ...
    def predict(self, question: str) -> SLMResponse:
        ...
        if not answer and self._ds:
            try:
                ...
                logger.debug(f"ArtSLM DataSource query: {e}")
