"""
[OPT-17] LawSLM — DomainExpert for law (uses existing LegalDataSource).
...
NOTE: Distinct from existing LegalSLM (_DomainSLM) — this version has explicit
...
class LawSLM(BaseSLM):
    ...
    def __init__(self, config: Optional[dict] = None):
        super().__init__(name="LawSLM", domain="law", config=config)
        self._ds = None
        try:
            from scp.data_sources.legal import LegalDataSource
            self._ds = LegalDataSource()
        except Exception as e:
            logger.debug(f"LawSLM LegalDataSource init: {e}")
    ...
    def predict(self, question: str) -> SLMResponse:
        ...
        if not answer and self._ds:
            try:
                ...
                logger.debug(f"LawSLM DataSource query: {e}")
