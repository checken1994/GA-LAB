"""
[OPT-13] EconomicsSLM — DomainExpert for economics (uses FRED + WorldBank).
...
Pattern: BaseSLM subclass with local knowledge dict + DataSource fallback.
"""
...
class EconomicsSLM(BaseSLM):
    ...
    def __init__(self, config: Optional[dict] = None):
        super().__init__(name="EconomicsSLM", domain="economics", config=config)
        ...
        if not answer:
            ...
            if self._fred and self._fred.enabled:
                try:
                    ...
                    logger.debug(f"EconomicsSLM FRED query: {e}")
            ...
            if not answer and self._worldbank and self._worldbank.enabled:
                try:
                    ...
                    logger.debug(f"EconomicsSLM WorldBank query: {e}")
