"""
[OPT-19] MilitarySLM — DomainExpert for military (uses existing MilitaryDataSource).
...
NOTE: Distinct from existing MilitarySLM (_DomainSLM) — this version has
...
class MilitarySLM(BaseSLM):
    ...
    def __init__(self, config: Optional[dict] = None):
        super().__init__(name="MilitarySLM", domain="military", config=config)
        self._ds = None
        try:
            from scp.data_sources.military import MilitaryDataSource
            self._ds = MilitaryDataSource()
        except Exception as e:
            logger.debug(f"MilitarySLM MilitaryDataSource init: {e}")
    ...
    def predict(self, question: str) -> SLMResponse:
        ...
        if not answer and self._ds:
            try:
                ...
                logger.debug(f"MilitarySLM DataSource query: {e}")
