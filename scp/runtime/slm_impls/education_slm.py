"""
[OPT-18] EducationSLM — DomainExpert for education (uses ERIC DataSource).
...
NOTE: Distinct from existing EducationSLM (_DomainSLM) — this version uses
...
class EducationSLM(BaseSLM):
    ...
    def __init__(self, config: Optional[dict] = None):
        super().__init__(name="EducationSLM", domain="education", config=config)
        self._eric = None
        try:
            from scp.data_sources.eric import ERICDataSource
            self._eric = ERICDataSource()
        except Exception as e:
            logger.debug(f"EducationSLM ERIC init: {e}")
    ...
    def predict(self, question: str) -> SLMResponse:
        ...
        if not answer and self._eric and self._eric.enabled:
            try:
                ...
                logger.debug(f"EducationSLM ERIC query: {e}")
