...
class Finance(Base):
    ...
    def __init__(self, config: Optional[dict] = None, registry = None):
        super().__init__(name="Fin", domain="finance", config=config)
        self.registry = registry or None
        # [V29 FIX] Removed deprecated extractor — using direct API calls like Conversion
    ...
    def predict(self, question: str) -> SLMResponse:
        ...
        try:
            ...
            cached = cache.get("slm:Finance", question)
        ...
        try:
            ...
            cache.set("slm:Finance", question, resp, source)
        except Exception:
...
class Weather(Base):
    ...
    def predict(self, question: str) -> SLMResponse:
        ...
        try:
            from scp.core.smart_cache import get_smart_cache
            cached = get_smart_cache().get("slm:Weather", question)
            if cached is not None:
                self._end_timer(start, True)
                return cached
        except Exception:
...
class Logic(Base):
    ...
    def predict(self, question: str) -> SLMResponse:
        ...
        try:
            ...
            cached = slm_cache_get("Logic", question)
            if cached is not None:
                self._end_timer(start, True)
                return cached
        except Exception:
        ...
        try:
            ...
            slm_cache_set("Logic", question, resp, evidence.get("source", "PythonAST"))
        except Exception:
...
class Statistics(Base):
    ...
    def __init__(self, config: Optional[dict] = None):
        super().__init__(name="Stats", domain="statistics", config=config)
    ...
    def predict(self, question: str) -> SLMResponse:
        ...
        try:
            from scp.core.smart_cache import slm_cache_get, slm_cache_set
            cached = slm_cache_get("Stats", question)
            if cached is not None:
                self._end_timer(start, True)
                return cached
        except Exception:
        ...
        try:
            from scp.core.smart_cache import slm_cache_set
            slm_cache_set("Stats", question, resp, evidence.get("source", "PythonMath"))
        except Exception:
