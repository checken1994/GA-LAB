...
class Math(Base):
    ...
    def __init__(self, config: Optional[dict] = None):
        super().__init__(name="Math", domain="math", config=config)
    ...
    def predict(self, question: str) -> SLMResponse:
        ...
        try:
            ...
            cached = slm_cache_get("Math", question)
        ...
        # `except Exception` → Math returns 0.0-confidence error for ALL
        ...
        try:
            ...
            slm_cache_set("Math", question, resp, "PythonAST")
        except Exception:
...
class Biology(Base):
    ...
    def __init__(self, config: Optional[dict] = None, registry = None):
        super().__init__(name="Bio", domain="biology", config=config)
    ...
    def predict(self, question: str) -> SLMResponse:
        ...
        try:
            from scp.core.smart_cache import slm_cache_get, slm_cache_set
            cached = slm_cache_get("Bio", question)
            if cached is not None:
                self._end_timer(start, True)
                return cached
        except Exception:
        ...
        # Was: Biology only knows human anatomy facts
        ...
        try:
            from scp.core.smart_cache import slm_cache_set
            slm_cache_set("Bio", question, resp, evidence.get("source", "InternalKB"))
        except Exception:
