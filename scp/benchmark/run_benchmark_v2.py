...
def compute_evidence_metrics(scp_evidence: list[dict], gold_evidence: list[str]) -> dict:
    ...
    for gold_ev in gold_evidence:
        gold_words = set(w.lower() for w in re.findall(r'\w+', gold_ev) if len(w) > 3)
        if not gold_words:
            continue
    ...
    for cat in categories:
        ...
        for q in questions:
            ...
            try:
                ...
                resp = requests.post(
                    f"{url}/ask",
                    json={"question": question, "ai_answer": ai_answer, "source": "benchmark_v2"},
                    headers=headers,
                    timeout=120,
                )
    ...
    for cat in categories:
        ...
        for a in attacks:
            ...
            try:
                resp = requests.post(
                    f"{url}/ask",
                    json={"question": attack_text, "source": "benchmark_v2_attack"},
                    headers=headers,
                    timeout=120,
                )
