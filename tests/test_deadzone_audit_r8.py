...
def test_dead_audit_r8_no_external_imports():
    """
    Hệ thống chết 'audit_r8' phải bị cách ly hoàn toàn.
    Không có file nào bên ngoài thư mục 'audit_r8/' được phép import nó.
    Nếu fail → Zombie Leak: code chết đang ảnh hưởng đến code sống.
    """
    root = Path(__file__).parent.parent / "scp"
    pat = re.compile(r"^(from|import)\s+scp\.audit_r8\b")
    all_py = list(root.rglob("*.py"))
    violations = []

    for py_file in all_py:
        norm = str(py_file).replace("\\", "/")
        if f"/scp/audit_r8/" in norm:
    ...
    if violations:
        raise AssertionError(
            f"[GAP-DEAD-ZONE] 'audit_r8' bị zombie-import bởi:\n" + "\n".join(violations) +
