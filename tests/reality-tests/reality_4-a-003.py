...
Before fix: All 3 route file docstrings claimed "DEAD ROUTE — not registered"
but api_server.py:589-611 DOES register all 3 routers. None of the route
decorators had `Depends(verify_admin)` — state-changing POST endpoints
...
DNA principles exercised:
  ...
  #9  (no harm — admin gate closes an unauthenticated state-changing POST)
  #22 (PASS ≠ TRUE — docstring lied "DEAD ROUTE" while router was live)
  #26 (reality test — execute + cross-check, not just trust the comment)
"""
import os
import sys
...
# ---------------------------------------------------------------------------
# TEST 1 — All 3 route files exist (DNA #19: observation — they're real files)
# ---------------------------------------------------------------------------
for name in FILES:
...
# ---------------------------------------------------------------------------
# TEST 2 — No "DEAD ROUTE" in any of the 3 docstrings (DNA #22: docstring must
# not lie — reality (api_server.py registers them) must match the docstring).
# We allow "Live — admin auth required" as the honest replacement marker.
# ---------------------------------------------------------------------------
for name in FILES:
    path = os.path.join(ROUTES_DIR, name)
    with open(path) as f:
    ...
    assert "DEAD ROUTE" not in content, (
        ...
        f"'Live — admin auth required (Fix 4-a-003)'."
    )
    assert "Live — admin auth required (Fix 4-a-003)" in content, (
        f"FAIL: {name} does not contain the honest replacement marker "
        f"'Live — admin auth required (Fix 4-a-003)'."
    )
print("PASS [2/3]: no 'DEAD ROUTE' lie in any of the 3 docstrings; honest 'Live' marker present")

# ---------------------------------------------------------------------------
# TEST 3 — Every @router.get/post decorator in each file has
# `dependencies=[Depends(verify_admin)]` (DNA #9: no harm — auth gate).
...
for name in FILES:
    ...
    for m in matches:
        ...
        # statement — decorators may span multiple lines until the closing )
        start = m.start()
...
print("\n✓ Reality test 4-a-003 PASSED (3/3 assertions)")
