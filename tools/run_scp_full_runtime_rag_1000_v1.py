...
ROOT=Path(r'C:\Users\check\Downloads\scp');SRC=ROOT/'data'/'benchmark_batches'/'cc047e32d62448678a773738abe08833'/'questions.jsonl';OUT=ROOT/'reports'/'SCP_FULL_RUNTIME_RAG_1000_2026-08-17.jsonl';OUT.parent.mkdir(parents=True,exist_ok=True)
seen=set()
if OUT.exists():
...
with OUT.open('a',encoding='utf-8') as f:
 for i,x in enumerate(rows,1):
  ...
  try:
   r=requests.post('http://127.0.0.1:8001/ask',json=body,timeout=240);rec['http_status']=r.status_code;data=r.json();rec.update({k:data.get(k) for k in ('verdict','run_status','confidence','governance_decision','final_answer','run_id','trace_id','elapsed_ms','v100_claims')});rec['error']=None
